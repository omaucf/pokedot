...
const QUICK_OPEN_INTERVAL := 400
const MULTILINE_TAB_BAR: PackedScene = preload("res://addons/editor.script-ide/src/tap_bar/multiline_tab_bar.tscn")
const MultilineTabBar := preload("res://addons/editor.script-ide/src/tap_bar/multiline_tab_bar.gd")
const QUICK_OPEN_SCENE: PackedScene = preload("res://addons/editor.script-ide/src/quick_open/quick_open_popup.tscn")
const QuickOpenPopup := preload("res://addons/editor.script-ide/src/quick_open/quick_open_popup.gd")
const OVERRIDE_SCENE: PackedScene = preload("res://addons/editor.script-ide/src/override/override_popup.tscn")
const OverridePopup := preload("res://addons/editor.script-ide/src/override/override_popup.gd")
const OUTLINE_CONTAINER_SCENE: PackedScene = preload("res://addons/editor.script-ide/src/outline/outline_container.tscn")
const OutlineContainer := preload("res://addons/editor.script-ide/src/outline/outline_container.gd")
const SplitCodeEdit := preload("res://addons/editor.script-ide/src/split_code_edit.gd")
...
static func find_or_null(arr: Array[Node]) -> Control:
	if (arr.is_empty()):
		push_error("Node that is needed for Script-IDE not found. Plugin will not work correctly.")
		return null
...
func on_tab_changed(_index: int):
...
func is_current_script_gd() -> bool:
	var script = get_current_script()
	return script is GDScript


func update_gdscript_ui():
	var is_gd := is_current_script_gd()

	if files_panel != null:
		files_panel.visible = is_gd
